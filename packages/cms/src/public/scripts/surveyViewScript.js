  ...
  if (surveyId === 0) {
    ...
    Sortable.create(el, {
      ...
      onSort: function (/**Event*/ evt) {
        ...
        $($('.question_parent')[$('.question_parent').length - 1])
          .find('.next_question_field')
          .attr('disabled', true)
          .val()
    ...
    .map((question, index) => {
      ...
      input.map((_, field) => {
        if (field.type == 'checkbox') {
          ...
          if (field.dataset.length)
            field.nextElementSibling.nextElementSibling.textContent = `${
              parseInt(field.dataset.length) - field.value.length
            } characters remaining.`
          if (!['Question', 'Response'].includes(field.name) && !question.is_multiple)
            field.disabled = true
        }
      })
      if (next_question) {
...
$('#btnEditConfirm').on('click', (e) => {
  ...
  $('.question_parent.new_question').map((index) => {
    ...
    let next_question = {}
    let isValid = false
    $($('.question_parent.new_question')[index])
      .find('.input_field')
      .map((_, item) => {
        if (item.type == 'checkbox')
          inputs = { ...inputs, [item.name.toLocaleLowerCase().split(' ').join('')]: item.checked }
        else {
          inputs = { ...inputs, [item.name.toLocaleLowerCase().split(' ').join('')]: item.value }
          if (item.name == 'Question' && !item.value && !item.disabled)
          ...
          if (item.name != 'Question' && item.name != 'Response') {
            if (
              item.value &&
              !item.disabled &&
              $(item.parentElement).find('.next_question_field').val() &&
              $(item.parentElement).find('.next_question_field').val() <
                item.parentElement.parentElement.rowIndex &&
              index < $('.question_parent.new_question').length - 1
            ) {
            ...
            if (
              ...
              $(item.parentElement)
                .find('.next_question_field')
                .val(item.parentElement.parentElement.rowIndex)
            }
            next_question = {
              ...next_question,
              [item.name.toLocaleLowerCase().split(' ').join('')]: $(item.parentElement)
                .find('.next_question_field')
                .val(),
            }
          }
    ...
    if (id != '')
      questions = [...questions, { ...inputs, response: '', sort_number: index, next_question, id }]
...
$(document).on('click', '#downloadSurveySigleCsvData', function (e) {
  var answeredSurveys = filteredSurveyData.length
    ? filteredSurveyData
    : JSON.parse($('#answeredSurveysJSON').text())
  var surveyID = e.currentTarget.dataset.value
  var surveyAnsweredList = answeredSurveys.filter((item) => {
    return item.id === surveyID
  })
  var surveys = JSON.parse($('#surveysJSON').text())
  const relevantSurvey = surveys.find((sur) => sur.id === surveyID)
  var rows = []
  ...
  surveyAnsweredList.map((surveyAnswered) => {
    ...
    if (surveyAnswered && surveyAnswered.questions) {
      ...
      questions
        .sort((a, b) => a.sort_number - b.sort_number)
        .map((question) => {
          let answer = (question.answer ?? '').toString().trim()

          if (!answer) {
            const answerIdRaw = question.answerID
            const questionId = question.questionId
            const selectedQuestion = relevantSurvey.questions.find((que) => que.id == questionId)

            if (selectedQuestion != null && answerIdRaw != null && answerIdRaw !== '') {
              const idx0 = Number(answerIdRaw) // zero-based index from data
              if (Number.isInteger(idx0) && idx0 >= 0) {
                const optKey = `option${idx0 + 1}` // 0 -> option1, 1 -> option2, ...
                if (Object.prototype.hasOwnProperty.call(selectedQuestion, optKey)) {
                  const optVal = (selectedQuestion[optKey] ?? '').toString().trim()
                  if (optVal !== '') {
                    answer = optVal
                  }
                }
              ...
              // Fallback if option column missing/empty: use free-text response if available
              if (!answer) {
                const resp = (question.response ?? '').toString().trim()
                if (resp) answer = resp
              }
            }
          }
          rows.push([
            userID,
            surveyAnswered.gender,
            surveyAnswered.location,
            surveyAnswered.country,
            AGE_GROUP_DATA[age_group] || 'N/A',
            question.question,
            answer,
          ])
        })
  ...
  $($($(this).parents()[3]).find('.input_field')).map((_, input) => {
    if (checked) {
      ...
      if (input.name.includes('Option')) {
        ...
        if (input.parentElement.parentElement.rowIndex - 1 < $('.new_question').length)
          $(input.parentElement)
            .find('.next_question_field')
            .val(input.parentElement.parentElement.rowIndex)
...
$(document).on('click', '.remove__question', function (e) {
  ...
  if ($(que).find('.id_input').val())
    deletedQuestion = [...deletedQuestion, $(que).find('.id_input').val()]
  $(que).remove()
  $($('.question_parent')[$('.question_parent').length - 1])
    .find('.next_question_field')
    .attr('disabled', true)
...
$(document).on('keydown', '.question_parent .validate__input', function (e) {
  ...
  if (
    e.target.value.length >= totalCount &&
    e.originalEvent.key.length === 1 &&
...
$('.filter__form_select').on('change', (e) => {
  ...
  filteredSurveyData = answeredSurveys.filter(
    (e) =>
      (!gender || e.gender == gender) &&
      (!age || parseInt(e[age])) &&
      (!location_val || e.location == location_val),
  )
